# Agent guide: TrustedRouterOnPrem

TrustedRouterOnPrem builds immutable task evidence from agent traces, composes and fits frozen model routers,
...
- `tr-onprem build PROJECT --traces TRACE_FILE --source SOURCE --root ROOT` is the only CLI path
...
- `tr-onprem optimize router PROJECT --root ROOT` discovers the completed build, fit-only RAG, grounded
...
- `tr-onprem` opens the branded home screen. Its `Run Gateway` choice starts the initialized
  authenticated multi-alias gateway on loopback, running first-time setup when no gateway exists,
  while `tr-onprem --project PROJECT --root ROOT --port
  ...
  continuations use `previous_response_id`. TrustedRouterOnPrem never joins unrelated Chat callers by transcript
...
- No-subcommand `tr-onprem` serves only explicit active gateway aliases. The `--project` form serves one
...
- `tr-onprem optimize model PROJECT` runs only a project-bound immutable W12 to W13 SFT configuration.
...
5. **The top level is a closed allowlist.** The tracked top-level directories are exactly: `exp/`,
   ...
   **Agents must never create a new top-level directory.** Not for scratch work, not for a
   ...
   - `docs/`: **reviewed public documentation** in `docs/research/` (completed research writeups
     ...
     Reproduction lives in the report itself, quoted as public `exp` Python API and `tr-onprem` CLI plus the exact
     parameter pins.
...
6. **Benchmark data is external input, not a repository directory.** Give `tr-onprem build` one explicit
...
9. **Design every public surface from the perspective of a dev using it.** Before implementing a
   ...
   `exp` Python API, `tr-onprem` CLI commands, pydantic models) stay small, composable, and explicitly typed.
...
This repo publishes **one flagship distribution**: `trustedrouter-onprem`, whose importable Python code
...
- **Publishing**: `.github/workflows/python-package.yml` builds and publishes the flagship
  `trustedrouter-onprem` distribution and the `exp-gateway-native` abi3 wheel matrix; the publish job
