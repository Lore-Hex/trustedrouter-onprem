...
|---|---|---|
| `tr-onprem` | Open the branded home screen. `Run Gateway` is the first option and runs setup when needed. | Interactive gateway menu, or the default gateway in a non-interactive terminal. |
| `tr-onprem login [--root ROOT]` | Read a TrustedRouter API key from the environment or hidden input, save it, and synchronize the account's model identities. | User-local credential plus secret-free hosted provider/model records in `.exp/models.toml`. |
| `tr-onprem run [PROJECT] [--root ROOT] [--check]` | Start the local gateway directly, optionally with one project-backed alias. | OpenAI-compatible endpoint, readiness routes, and content-free usage view. |
| `tr-onprem build PROJECT [-t PATH] --source SOURCE --root ROOT [--provider NAME ...]` | Launch the guided end-to-end build when traces are omitted, or use one explicit local source for automation. | Simulation, serving RAG, fit RAG, syllabus, evaluation evidence, and a runnable automatic router. |
| `tr-onprem optimize router PROJECT --root ROOT [--yes]` | Complete bounded simulation and judgment, fit a frozen router, then verify held-out evidence. | Fit evaluation, policy, held-out evaluation, and router report. |
| `tr-onprem optimize model PROJECT --root ROOT [--yes]` | Verify one project-bound W12 dataset and conservatively preflight bounded managed Tinker SFT. | Completed W13 result and registered frozen alias, or a fail-closed preflight with no paid dispatch. |
| `tr-onprem --root ROOT [--check]` | Validate or start the initialized authenticated default gateway on loopback; the native data plane serves every route, including Chat Completions, Responses, and Anthropic Messages. | OpenAI-compatible and Anthropic Messages endpoints, readiness routes, and content-free usage view. |
| `tr-onprem --project PROJECT --root ROOT [--ghost]` | Activate a frozen policy as one project-backed alias and launch the normal gateway. | The same authenticated OpenAI endpoint and SQLite accounting as the default gateway. |
| `tr-onprem config gateway ...` | Author provider references, identities, virtual keys, grants, aliases, certified exact-model pools, monthly limits, status, and usage without optimizer roles. | Private SQLite authority, immutable catalog snapshots, and versioned receipts. |
...
`tr-onprem config budget USD --root ROOT`. `--yes` confirms only an in-budget invocation and never raises
the ceiling. Exact completed replays report a zero-dollar estimate and do not prompt.

Product telemetry is disabled by default and has no bundled destination credential. An operator
must configure and explicitly enable it before aggregate metadata can leave the machine. Gateway
...
while preserving existing identities, keys, grants, usage, and history. `tr-onprem --non-interactive --json` returns `gateway_not_initialized` plus exact
next commands on an empty root. `tr-onprem --check` validates local readiness without binding.
First-run setup prints `TRUSTEDROUTER_ONPREM_URL` and the newly issued `TRUSTEDROUTER_ONPREM_KEY` before readiness is
...
unavailable alias and provider configuration; fix that configuration and rerun `tr-onprem`. If the
one-time key was not saved, issue a replacement with
`tr-onprem config gateway key issue IDENTITY --key-id KEY --json`.
...
value. Interactive `tr-onprem config providers` persists a pasted key in the platform user-data file
...
`tr-onprem config providers`. Bedrock stays on the AWS credential chain. Current provider revisions
...
```console
tr-onprem config gateway pool certify PUBLIC_ALIAS \
...
Monthly serving limits are separate from the one-shot `tr-onprem config budget` command ceiling. They use
...
```console
tr-onprem config gateway budget set --period 2026-08 --scope identity \
  --identity TEAM_MEMBER --limit-micro-usd 20000000000 \
  --root ROOT --non-interactive --json
tr-onprem config gateway budget set --period 2026-08 --scope deployment \
...
tr-onprem config gateway budget set --period 2026-08 --scope deployment \
  ...
  --limit-micro-usd 10000000000 --root ROOT --non-interactive --json
tr-onprem config gateway budget remaining --period 2026-08 --root ROOT --json
```
...
after `--replace` raises it, and `tr-onprem config gateway budget reconcile --period 2026-08 --scope
...
[`router-optimization.json`](reference/router_optimization_config.md) with `tr-onprem optimize router`.
