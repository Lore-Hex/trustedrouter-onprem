...
- `tr-onprem login` resolves a TrustedRouter key from `TRUSTEDROUTER_API_KEY` or a masked prompt,
  stores it in the user-data credential file, and synchronizes the authenticated account's hosted
  provider and model identities into the secret-free project catalog. No credential value is
...
- The no-subcommand default gateway launch, the direct `tr-onprem run [PROJECT]` form, and the
  `tr-onprem --project PROJECT [--ghost]` compatibility form are installed-wheel surfaces. Gateway
