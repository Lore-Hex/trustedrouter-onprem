...
`tr-onprem` opens the gateway home screen. Its `Run Gateway` choice starts an authenticated
...
`tr-onprem --project PROJECT` is compatibility sugar that activates one project-backed alias and launches
...
compatible provider connections), `gemini_generate_content`, and
...
`tr-onprem config gateway` owns explicit local setup. Its provider, identity, key, grant, alias, pool, and
...
A singleton alias creates a one-deployment pool. `tr-onprem config gateway pool certify` can replace that
...
Azure and compatible endpoints share compatible-stream coverage; Gemini and Bedrock have native deterministic
