...
`tr-onprem build PROJECT --traces PATH --source SOURCE` reads one explicit local corpus through one canonical
...
Raw exports remain at the customer path. TrustedRouterOnPrem stores a normalized immutable snapshot and explicit
...
`tr-onprem config telemetry disable`. Telemetry does not include prompts, traces, paths, model names, or
customer content.
...
`tr-onprem build` local-file boundary and requires separate authorization for the customer PostHog
project.
