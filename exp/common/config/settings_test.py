"""Tests for project-local telemetry and command-budget settings under `.exp/settings.toml`."""
...
def test_missing_settings_defaults_to_telemetry_enabled(tmp_path: Path) -> None:
    settings = load_settings(tmp_path / ".exp")
    ...
    assert resolve_command_budget_usd(tmp_path / ".exp", None) == DEFAULT_COMMAND_BUDGET_USD
