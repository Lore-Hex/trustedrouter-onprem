...
from exp.common.config.settings import ensure_telemetry_anonymous_id, load_settings
from exp.common.core.artifacts import canonical_json_bytes, sha256_json, validate_artifact_id
...
_ALLOWED_EVENT_PROPERTIES: dict[str, frozenset[str]] = {
    ...
    "exp router completed": frozenset(
        {
            "success",
            "fit_cell_count",
            "heldout_cell_count",
    ...
    "exp simulation completed": frozenset(
        {
            "success",
            "rollout_count",
            "duration_seconds",
...
def _completion_event_uuid(event: str, completion_id: str) -> UUID:
    ...
    return uuid5(NAMESPACE_URL, f"trustedrouter-onprem:{event}:{completion_id}")
...
def _exp_version() -> str:
    try:
        return version("trustedrouter-onprem")
