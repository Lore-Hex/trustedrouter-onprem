...
from exp.cli.providers.trustedrouter import SETUP_PICKER_LABEL
...
def test_gateway_provider_selector_accepts_multiple_providers() -> None:
    ...
    console = ScriptedConsole("1,2,5,6\n\n")
    ...
    assert selected == (("trustedrouter", "openai", "openai-compatible", "azure"), True)
...
def test_gateway_provider_selector_uses_the_builder_keyboard_picker() -> None:
    ...
    keys = iter(
        (
            *(PickerKey.DOWN for _ in range(4)),
            PickerKey.ENTER,
...
def test_gateway_setup_marks_trustedrouter_deployment_as_host_managed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        setup,
        "select_providers",
        lambda *_args, **_kwargs: (("trustedrouter",), False),
    )
    monkeypatch.setattr(
        setup,
        "prepare_providers",
...
def test_trustedrouter_connection_uses_the_hosted_gateway() -> None:
    """TrustedRouter does not collect a local base URL or credential name."""
    connection = provider_picker.collect_provider_connection(
        "trustedrouter",
        console=ScriptedConsole(""),
        environment={},
