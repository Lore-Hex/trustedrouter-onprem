...
def test_first_run_delivers_credentials_before_readiness_failure(
    ...
    raw_key = "trp_vk_test-first-run-key"
    output = io.StringIO()
    ...
    def load_components(
        ...
        *,
        ...
        assert transcript.index(f"export TRUSTEDROUTER_ONPREM_KEY={raw_key}") < len(transcript)
        raise ValueError(
    ...
    assert "export TRUSTEDROUTER_ONPREM_URL=http://127.0.0.1:8000/v1" in transcript
    assert f"export TRUSTEDROUTER_ONPREM_KEY={raw_key}" in transcript
...
def test_first_run_unknown_setup_outcome_delivers_the_only_raw_key(
    ...
    transcript = output.getvalue()
    assert "export TRUSTEDROUTER_ONPREM_KEY=trp_vk_unknown_secret" in transcript
    assert "Preserve this one-time gateway key: trp_vk_unknown_secret" in transcript
    assert "exp config gateway key issue default --key-id RECOVERY_KEY --json" in transcript
