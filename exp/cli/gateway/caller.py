...
_URL_OPTION = typer.Option(
    DEFAULT_GATEWAY_URL,
    "--url",
    envvar=["TRUSTEDROUTER_ONPREM_URL", "OPENAI_BASE_URL"],
    help="Base URL of a live gateway, normally ending in /v1.",
)
_KEY_OPTION = typer.Option(
    None,
    "--key",
    envvar=["TRUSTEDROUTER_ONPREM_KEY", "OPENAI_API_KEY"],
...
def _require_key(key: str | None) -> str:
    ...
    raise typer.BadParameter(
        "a virtual gateway key is required; pass --key, set TRUSTEDROUTER_ONPREM_KEY or "
...
def _unreachable_gateway(url: str) -> typer.BadParameter:
    ...
    return typer.BadParameter(
        ...
        "--url (or TRUSTEDROUTER_ONPREM_URL) at a live gateway"
    )
