...
def _emit_setup_credentials(
    *,
    ...
    output.print(f"export TRUSTEDROUTER_ONPREM_URL=http://{_LOOPBACK_HOST}:{port}/v1", markup=False)
    output.print(f"export TRUSTEDROUTER_ONPREM_KEY={setup.raw_key}", markup=False)
...
def _emit_gateway_ready(
    *,
    ...
    if compatibility is not None:
        ...
        _console.print(
            f"export TRUSTEDROUTER_ONPREM_URL=http://{_LOOPBACK_HOST}:{port}/v1",
            markup=False,
        )
        _console.print(
            f"export TRUSTEDROUTER_ONPREM_KEY=\"$(tr -d '\\n' < {compatibility.key_file})\"",
            markup=False,
        )
