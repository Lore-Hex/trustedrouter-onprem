...
def test_home_screen_starts_with_brand_and_puts_run_gateway_first(tmp_path: Path) -> None:
    ...
    assert "TrustedRouterOnPrem gateway" in console.output
...
def test_setup_gateway_returns_to_home_with_one_time_credentials(
    ...
    assert "export TRUSTEDROUTER_ONPREM_URL=http://127.0.0.1:8000/v1" in console.output
    assert "export TRUSTEDROUTER_ONPREM_KEY=trp_vk_test" in console.output
...
def test_setup_gateway_warns_and_reconfigures_an_initialized_gateway(
    ...
    assert "export TRUSTEDROUTER_ONPREM_KEY=trp_vk_reconfigured" in console.output
...
def test_setup_gateway_preserves_a_key_when_reconfiguration_outcome_is_unknown(
    ...
    assert "Preserve this one-time gateway key: trp_vk_test_secret" in console.output
