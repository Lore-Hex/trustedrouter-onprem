...
def test_noninteractive_setup_collects_many_connections_models_and_roles(tmp_path: Path) -> None:
    ...
    Args:
        tmp_path: Temporary EXP root receiving the model catalog.
    """
    root = tmp_path / ".exp"
...
def test_noninteractive_provider_flags_validate_without_prompts_or_writes(tmp_path: Path) -> None:
    ...
    Args:
        tmp_path: Temporary EXP root without provider configuration.
    """
    root = tmp_path / ".exp"
    result = _RUNNER.invoke(
        app,
    ...
    assert (
        "choose from: trustedrouter, openai, anthropic, gemini, "
...
def test_explicit_providers_skip_the_opening_list_and_still_discover_models(
    ...
    Args:
        tmp_path: Temporary EXP root receiving the saved catalog.
...
def test_noninteractive_setup_reports_every_missing_collection_and_role(tmp_path: Path) -> None:
    ...
    result = _RUNNER.invoke(
        app,
        ["config", "providers", "--root", str(tmp_path / ".exp"), "--non-interactive"],
        color=True,
    )
...
def test_setup_preserves_router_candidates_and_unrelated_entries(tmp_path: Path) -> None:
    ...
    Args:
        tmp_path: Temporary EXP root containing preserved catalog state.
    """
    root = tmp_path / ".exp"
    root.mkdir()
    write_model_catalog(
...
def test_structured_input_rejects_openai_compatible_without_capabilities(tmp_path: Path) -> None:
    ...
    Args:
        tmp_path: Temporary EXP root receiving rejected structured input.
    """
...
def _setup(
    ...
    *,
    ...
    Args:
        root: Local EXP root receiving the catalog.
...
def test_wizard_recommended_setup_needs_only_provider_and_one_default_choice(
    ...
    Args:
        tmp_path: Temporary EXP root receiving the secret-free catalog.
...
def test_wizard_recommended_setup_prefers_provider_diversity_for_router_candidates(
    ...
    Args:
        tmp_path: Temporary EXP root receiving the selected multi-provider catalog.
...
def test_wizard_recommended_setup_falls_back_by_verified_capability_and_cost(
    ...
    Args:
        tmp_path: Temporary EXP root receiving deterministic fallback choices.
...
def test_interactive_final_rejection_writes_no_catalog(
    ...
    Args:
        tmp_path: Temporary EXP root used to prove no catalog write occurs.
        monkeypatch: Patch fixture supplying the canonical credential.
    """
    root = tmp_path / ".exp"
...
def test_setup_preserves_entries_owned_by_providers_it_does_not_configure(
    ...
    Args:
        tmp_path: Temporary EXP root containing a registered SFT sampling handle.
        monkeypatch: Patch fixture supplying the canonical credential.
    """
    root = tmp_path / ".exp"
    root.mkdir()
...
def test_configured_models_are_reassignable_without_any_provider_request(
    ...
    Args:
        tmp_path: Temporary EXP root containing a complete catalog.
...
def test_offline_roles_include_models_on_tinker_without_provider_requests(tmp_path: Path) -> None:
    ...
    Args:
        tmp_path: Temporary EXP root containing a complete custom-provider catalog.
    """
    root = tmp_path / ".exp"
    root.mkdir()
    chat = ModelCapabilities(
...
def test_offline_roles_retain_assigned_tinker_alias_without_capabilities(tmp_path: Path) -> None:
    ...
    Args:
        tmp_path: Temporary EXP root containing an assigned legacy Tinker alias.
    """
    root = tmp_path / ".exp"
    root.mkdir()
    embedding = ModelCapabilities(
...
def test_offline_setup_preserves_exact_unverified_router_roles_without_revalidation(
    ...
    Args:
        tmp_path: Temporary EXP root containing complete build roles and legacy candidates.
    """
    root = tmp_path / ".exp"
    root.mkdir()
    chat = ModelCapabilities(
