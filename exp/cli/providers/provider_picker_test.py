...
def test_setup_provider_surface_has_one_first_party_hosted_router() -> None:
    """Interactive setup exposes TrustedRouter, direct BYOK, and private endpoints only."""
    assert tuple(provider_picker.SETUP_PROVIDER_LABELS) == (
        "trustedrouter",
        "openai",
        "anthropic",
        "gemini",
    ...
    assert "openrouter" not in provider_picker.SETUP_PROVIDER_LABELS
...
def test_provider_screen_selects_several_providers_in_one_session() -> None:
    ...
    console = ScriptedConsole("1,2,3\n\n")

    selection = select_providers(
        SetupSession(), console=console, environment={"OPENAI_API_KEY": "secret-key"}
    )

    assert selection == (("trustedrouter", "openai", "anthropic"), False)
...
def test_trustedrouter_uses_masked_key_and_native_provider(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """TrustedRouter stores a pasted key and discovers through its native adapter."""
    monkeypatch.setattr(provider_picker, "getpass", lambda _prompt="": "sk-tr-v1-pasted")
    console = ScriptedConsole("")
    lister = _FakeLister(
        {
            "trustedrouter": [
    ...
    assert prepared is not None
    endpoints, _ = prepared
    assert endpoints[0].api_key == "sk-tr-v1-pasted"
    assert endpoints[0].connection.provider == "trustedrouter"
    assert endpoints[0].connection.base_url is None
    assert endpoints[0].connection.api_key_env == HOSTED_GATEWAY_API_KEY_ENV
    assert ProviderAuthStore(default_auth_path()).get("trustedrouter") == "sk-tr-v1-pasted"
    assert "sk-tr-v1-pasted" not in console.output
...
def test_release_tty_walk_selects_azure_then_completes() -> None:
    ...
    assert "TrustedRouter" in console.output
...
def test_trustedrouter_persists_the_native_hosted_lane(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """TrustedRouter writes its fixed-origin native provider identity."""
...
def test_trustedrouter_lists_through_the_native_provider() -> None:
    """Discovery uses the native provider so TrustedRouter catalog metadata applies."""
    console = ScriptedConsole("")
    lister = _FakeLister(
        {
            "trustedrouter": [
    ...
    prepared = _prepare(
        ...
        providers=("trustedrouter",),
        lister=lister,
        environment={HOSTED_GATEWAY_API_KEY_ENV: "sk-tr-v1-test_key"},
    )

    assert prepared is not None
    endpoints, models = prepared
    assert len(endpoints) == 1
    assert endpoints[0].connection.name == "trustedrouter"
    assert endpoints[0].connection.provider == "trustedrouter"
    assert endpoints[0].connection.base_url is None
    assert endpoints[0].connection.api_key_env == HOSTED_GATEWAY_API_KEY_ENV
    assert lister.requests == [
