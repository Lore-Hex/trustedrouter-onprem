...
REQUIRED_WHEEL_MODULES = frozenset(
    {
        ...
        "exp/simulation/comparison.py",
        "exp/simulation/engines/sandbox.py",
        "exp/optimize/router/automatic/service.py",
        "exp/optimize/router/composition.py",
        "exp/optimize/router/evaluation/setup.py",
        "exp/optimize/router/fit/workflow.py",
        "exp/optimize/router/judging/service.py",
        "exp/optimize/router/judging/review.py",
        "exp/optimize/router/judgment_budget.py",
    }
)
REQUIRED_SDIST_MEMBERS = frozenset(
    {
        "README.md",
        "assets/trustedrouter-onprem-workflow.png",
...
FORBIDDEN_FLAT_ROUTER_MODULES = frozenset(
    {
        "exp/optimize/router/automatic_router.py",
        "exp/optimize/router/automatic_router_artifacts.py",
        "exp/optimize/router/automatic_router_judge.py",
        "exp/optimize/router/automatic_router_preflight.py",
        "exp/optimize/router/automatic_router_replay.py",
        "exp/optimize/router/automatic_router_reservations.py",
        "exp/optimize/router/completed_build.py",
        "exp/optimize/router/manual_judge.py",
        "exp/optimize/router/manual_judge_artifacts.py",
        "exp/optimize/router/manual_judge_contracts.py",
        "exp/optimize/router/manual_judge_protocol.py",
        "exp/optimize/router/manual_judge_selection.py",
        "exp/optimize/router/optimizer.py",
        "exp/optimize/router/persistence.py",
        "exp/optimize/router/report.py",
        "exp/optimize/router/router_attribution.py",
        "exp/optimize/router/router_execution_contract.py",
        "exp/optimize/router/router_setup.py",
        "exp/optimize/router/router_simulation_spec.py",
        "exp/optimize/router/simulation_spend.py",
        "exp/optimize/router/spec.py",
        "exp/optimize/router/workflow.py",
    }
)
...
def _normalized_path(member_name: str) -> str:
    ...
    if parts and parts[0].startswith(("trustedrouter-onprem-", "trustedrouter_onprem-")):
        parts = parts[1:]
...
def _assert_current_archive_members(
    ...
    *,
    ...
    removed_package = PurePosixPath("exp", "workflow").as_posix() + "/"
    ...
    if allow_tests:
        return
    tests = sorted(
        name for name in file_names if name.endswith("_test.py") or name == "exp/conftest.py"
    )
...
def _tracked_wheel_members() -> frozenset[str]:
    ...
    return frozenset(
        ...
        if name.startswith("exp/") and not name.endswith("_test.py") and name != "exp/conftest.py"
    )
...
def _installed_release_driver() -> None:
    ...
    from exp.common.routing.features import ROUTER_FEATURE_SCHEMA_SHA256
    ...
    source_checkout = Path(os.environ["EXP_SOURCE_CHECKOUT"]).resolve()
    ...
    package_path = Path(exp.__file__).resolve()
    assert "site-packages" in package_path.parts, package_path
    assert not package_path.is_relative_to(source_checkout)
    ...
    root = execution_root / ".exp"
    traces = execution_root / "support.otel.jsonl"
    one_trace = execution_root / "one.otel.jsonl"
    executable = Path(sys.executable).with_name("tr-onprem")
    child_environment = os.environ.copy()
    child_environment.update(
        {
            "EXP_RELEASE_REVISION": release_revision,
            "EXP_INSTALLED_RELEASE_EVIDENCE": "0",
        ...
        *,
        ...
        for index in range(count):
            ...
            common = [
                ...
                attribute("exp.model.capabilities_sha256", capabilities_sha256),
                attribute("exp.model.connection_sha256", azure_connection_sha256),
                attribute("exp.customer.id", f"customer-{index}"),
                attribute("exp.conversation.id", f"conversation-{index}"),
            ]
            records.append(
                {
                    "traceId": trace_id,
    ...
    assert raw_key.startswith("trp_vk_")
    ...
    setup_answers = [
        (
            "Providers",
            # TrustedRouter is first, then openai through openai-compatible and Azure;
    ...
    try:
        ...
        world_model = exp.load_world_model(
            "support-agent",
            root=root,
        ...
        try:
            ...
            assert compatibility_key.startswith("trp_vk_")
...
def test_installed_wheel_no_spend_release_evidence(tmp_path: Path) -> None:
    ...
    driver_environment.update(
        {
            "EXP_INSTALLED_RELEASE_EVIDENCE": "1",
            "EXP_RELEASE_REVISION": _TEST_RELEASE_REVISION,
            "EXP_TELEMETRY": "0",
            "PYTHONNOUSERSITE": "1",
            "EXP_SOURCE_CHECKOUT": str(repository),
        }
    )
    _run_checked(
...
def test_built_archives_match_current_package_contract() -> None:
    ...
    with zipfile.ZipFile(wheels[0]) as wheel:
        ...
        assert frozenset(name for name in names if name.startswith("exp/")) == (
...
def test_w16_public_evidence_apis_resolve_from_release_owners() -> None:
    ...
    from exp.common.judging import HumanScoreReview, JudgeCalibrationService, RubricReview
    from exp.runtime.environments import LocalProcessEnvironmentRuntime
    from exp.simulation import compare_text_and_sandbox
    from exp.simulation.engines import SandboxSimulator
    ...
    assert SandboxSimulator.__module__ == "exp.simulation.engines.sandbox"
    assert LocalProcessEnvironmentRuntime.__module__ == "exp.runtime.environments.local"
...
def test_documentation_index_commands_and_release_scope_are_current() -> None:
    ...
    assert "tr-onprem optimize router" in usage
    assert "tr-onprem optimize model" in usage
    assert "tr-onprem config gateway" in usage
    assert "tr-onprem config gateway pool certify" in usage
    assert "tr-onprem --root ROOT" in usage
    assert "OpenAI `3.0.0`" in usage
    assert "schema-v2" in usage
    assert "by_billing_source" in usage
    assert "tr-onprem optimize route" not in usage.replace("tr-onprem optimize router", "")
    assert "tr-onprem auth" not in usage
    assert "tr-onprem login" in usage
    assert "tr-onprem config credentials" not in usage
...
def test_public_setup_names_only_trustedrouter_as_a_managed_router() -> None:
    """Public onboarding fixes the managed route to TrustedRouter and carries no old branding."""
    repository = Path(__file__).resolve().parent.parent.parent
    public_paths = (
        repository / "README.md",
        repository / "SETUP.md",
        repository / "docs" / "reference" / "providers.md",
        repository / "docs" / "reference" / "openai-compatible-recipes.md",
    )
    public_text = "\n".join(path.read_text(encoding="utf-8") for path in public_paths)
    lowered = public_text.lower()

    assert "https://api.trustedrouter.com/v1" in public_text
    assert "trustedrouter_api_key" in lowered
    assert "openrouter" not in lowered
    assert "experiential cloud" not in lowered
    assert "experiential-cloud" not in lowered
    assert "api.quillrouter.com" not in lowered


if __name__ == "__main__" and os.environ.get("EXP_INSTALLED_RELEASE_EVIDENCE") == "1":
