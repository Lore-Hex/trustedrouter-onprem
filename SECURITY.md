...
This policy covers the code in this repository — the on-premise / self-hosted
deployment tooling for TrustedRouter. A self-hosted deployment runs on
infrastructure we do not control, so findings about **default configurations,
secret handling, network exposure, or privilege in the deployment path** are
especially in scope here.
...
The hosted service runs inside hardware-attested confidential computing
environments; published measurements and how to verify them are at
